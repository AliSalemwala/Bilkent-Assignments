...
% 21500413
%
% source code for dilation and erosion for the answer of question 1

%{
    Dilation definition: The structuring element is swept over
    the image. Each time the origin of the structuring element touches a
    binary 1-pixel, the entire translated structuring element is ORed
    to the output image, which was initialized to all zeros.
%}

%{
    Erosion definition: The structuring element is swept over
    the image. At each position where every 1-pixel of the structuring
    element covers a 1-pixel of the binary image, the binary image pixel
    corresponding to the origin of the structuring element is ORed to the 
    output image.
%}
...
for i = 1:size(rows)
    ...
    % OR the adjusted structuring element with the positions around the
    % current 1-pixel on the destination image
    [se_r, se_c] = size(se);
    temp = binary_image(r : r + se_r - 1, c: c + se_c - 1) | se;
    binary_image(r : r + se_r - 1, c: c + se_c - 1) = temp;
end
end
